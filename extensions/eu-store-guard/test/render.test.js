...
test('notice placement policies govern rendered HTML, including legacy configurations', () => {
  ...
  for (const [position, policy] of Object.entries(rule.presentation_policy.positions)) {
    ...
    const html = render(header ? strip('guarantee-notice-header.liquid') : NOTICE, {theme:{id:123},section:{id:'sections--123__test'},block:{id:'placement',settings:{position}},shop:{metafields:{eu_store_guard:{notice_status:mf('CONFIGURED'),...(header ? {notice_presentation:{value:{version:1,mechanism:'header-section',themeId:'123',sectionId:'sections--123__test',publicationReady:true}}} : {})}}},request:{locale:{iso_code:'es'}}});
