...
export async function handleNoticeApp(request, env, fetchImpl = (input, init) => fetch(input, init)) {
  ...
  try {
    ...
    if (url.pathname !== '/app/configuration' && !recheck && !observation && !migration) return json({ error: 'NOT_FOUND' }, 404);
