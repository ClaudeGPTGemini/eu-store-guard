...
function describe(status){return status==='CONFIGURED'?'Configuración guardada. Falta activar el aviso en el tema y comprobar su presentación. No se ha verificado la tienda.':status==='NOT_APPLICABLE'?'Aviso desactivado según los datos indicados.':'El aviso permanece desactivado. Falta completar o validar la configuración.';}
...
call('GET').then(d=>{form.elements.enabled.checked=d.input?.enabled===true;form.elements.goods.checked=d.input?.sellsGoodsToConsumers===true;message.textContent=describe(d.status);save.disabled=false;}).catch(error=>{const codes=['INVALID_SESSION','SHOPIFY_AUTH_FAILED','SHOP_OWNER_REQUIRED','SHOPIFY_REQUEST_FAILED','SHOPIFY_QUERY_REJECTED','SHOPIFY_IDENTITY_MISMATCH','METAFIELD_MIGRATION_REQUIRED','APP_REQUEST_FAILED','APP_NOT_CONFIGURED'];message.textContent='No se ha podido conectar con Shopify. Abre esta app desde la administración con la cuenta propietaria.'+(codes.includes(error.message)?' Diagnóstico: '+error.message+'.':'');});`;
...
export async function handleNoticeApp(request, env, fetchImpl = fetch) {
