import {acceptAssistedReview} from './notice-assisted-review.js';
...
import { checkSummary } from './notice-check-summary.js';
import { observePublishedTheme } from './notice-theme-observation.js';
import { noticeReviewSource, migrateDevReview } from './notice-review-store.js';
...
const approvalInput=document.querySelector('#review-file'),approvalMessage=document.querySelector('#review-message');
if(approvalInput)approvalInput.addEventListener('change',async()=>{const file=approvalInput.files?.[0];if(!file)return;try{if(file.size>4096)throw Error();const envelope=JSON.parse(await file.text());await call('POST',envelope,'/app/accept-review');approvalMessage.textContent='Revisión registrada. Guarda la configuración para comprobar el tema y aplicar el aviso.';}catch{approvalMessage.textContent='No se pudo aplicar la revisión. Puede haber caducado o no corresponder al estado actual de la tienda. Solicita una revisión nueva.';}finally{approvalInput.value='';}});
const inspect=document.querySelector('#inspect-theme'),inspection=document.querySelector('#theme-observation');
if(inspect)inspect.addEventListener('click',async()=>{inspect.disabled=true;try{inspection.textContent=JSON.stringify(await call('GET',undefined,'/app/theme-observation'),null,2);}catch{inspection.textContent='No se pudo leer el tema publicado. No se ha cambiado la configuración.';}finally{inspect.disabled=false;}});
...
function describeCheck(check){const warning=' No hay vigilancia continua: los cambios posteriores pueden no detectarse hasta volver a abrir la app o guardar. Si la revisión deja de ser válida, el aviso se desactiva al comprobarlo y requiere una nueva revisión antes de volver a publicarse. Esta comprobación no confirma que el aviso sea visible.';if(!check||typeof check.checkedAt!=='string'||!Number.isFinite(Date.parse(check.checkedAt)))return ' No hay una comprobación del tema registrada.'+warning;return ' Último intento de comprobación del tema: '+new Date(check.checkedAt).toLocaleString('es-ES')+'. '+(check.unchanged===true?'En ese momento no se detectaron cambios respecto a la revisión.':'No se confirmó que el tema conservara la revisión.')+warning;}
form.addEventListener('submit',async e=>{e.preventDefault();save.disabled=true;try{const d=await call('POST',{enabled:form.elements.enabled.checked,sellsGoodsToConsumers:form.elements.goods.checked,marketCountry:'ES',locale:'es'});message.textContent=describe(d.status)+describeCheck(d.themeCheck);}catch{message.textContent='No se pudo confirmar el guardado. Recarga la app y vuelve a intentarlo.';}finally{save.disabled=false;}});
call('GET').then(async d=>{form.elements.enabled.checked=d.input?.enabled===true;form.elements.goods.checked=d.input?.sellsGoodsToConsumers===true;const current=d.themeRecheckRequired?await call('POST',{},'/app/recheck-theme'):d;message.textContent=describe(current.status)+describeCheck(current.themeCheck);save.disabled=false;}).catch(error=>{const codes=['SHOPIFY_AUTH_UNREACHABLE','SHOPIFY_AUTH_INVALID_RESPONSE','SHOPIFY_ADMIN_UNREACHABLE','SHOPIFY_ADMIN_INVALID_RESPONSE','INVALID_SESSION','SHOPIFY_AUTH_FAILED','SHOP_OWNER_REQUIRED','SHOPIFY_REQUEST_FAILED','SHOPIFY_QUERY_REJECTED','SHOPIFY_IDENTITY_MISMATCH','METAFIELD_MIGRATION_REQUIRED','APP_REQUEST_FAILED','APP_NOT_CONFIGURED'];message.textContent='No se ha podido conectar con Shopify. Abre esta app desde la administración con la cuenta propietaria.'+(codes.includes(error.message)?' Diagnóstico: '+error.message+'.':'');});`;
...
export async function handleNoticeApp(request, env, fetchImpl = (input, init) => fetch(input, init)) {
  ...
  try {
    ...
    const requireThemeRevision=env.NOTICE_REVIEW_SOURCE === 'app-metafield' || env.NOTICE_THEME_RECHECK_ENABLED === 'true';
    ...
    const approval = url.pathname === '/app/accept-review';
    const migration = url.pathname === '/app/migrate-review';
    const observation = url.pathname === '/app/theme-observation';
    const recheck = url.pathname === '/app/recheck-theme';
    if (url.pathname !== '/app/configuration' && !recheck && !observation && !migration && !approval) return json({ error: 'NOT_FOUND' }, 404);
    if (approval && (request.method !== 'POST' || env.NOTICE_ASSISTED_REVIEW_ENABLED !== 'true')) return json({error:'ASSISTED_REVIEW_DISABLED'},503);
    if (migration && (request.method !== 'POST' || env.NOTICE_REVIEW_MIGRATION_ENABLED !== 'true')) return json({error:'REVIEW_MIGRATION_DISABLED'},503);
    if (observation && request.method !== 'GET') return json({error:'METHOD_NOT_ALLOWED'},405);
    if (recheck && (request.method !== 'POST' || !requireThemeRevision)) return json({error:'THEME_RECHECK_NOT_ENABLED'},503);
    ...
    if (approval) return json(await acceptAssistedReview(client,input,env));
    if (migration) {
      if (!input || typeof input !== 'object' || Array.isArray(input) || Object.keys(input).length) throw new AppError('INVALID_CONFIGURATION');
      return json(await migrateDevReview(client,env));
    }
    if (observation) return json(await observePublishedTheme(client));
    if (recheck) {
      if (!input || typeof input !== 'object' || Array.isArray(input) || Object.keys(input).length) throw new AppError('INVALID_CONFIGURATION');
      const deployment=noticeReviewSource(env);
      const result=await recheckNoticeTheme(client,deployment);
      return json({status:result.status,reason:result.config.decision.themeCheck.reason,themeCheck:checkSummary(result.config.decision.themeCheck),publicVerification:'pending'});
    }
    if (request.method === 'GET') {
      const snapshot = await client.read(); let input, stored;
      try { stored = JSON.parse(snapshot.currentAppInstallation.config?.value ?? 'null'); input=stored?.input; } catch { throw new AppError('METAFIELD_MIGRATION_REQUIRED', 409); }
      return json({ status: snapshot.shop.notice?.value ?? 'NEEDS_INFORMATION', input, themeCheck:checkSummary(stored?.decision?.themeCheck), themeRecheckRequired:requireThemeRevision && input?.enabled===true && !!snapshot.shop.presentation });
    }
    const deployment = noticeReviewSource(env);
    const result = await saveConfiguration(client, input, deployment, new Date(), requireThemeRevision);
    return json({ status: result.status, reasons: result.config.decision.reasons, themeCheck:checkSummary(result.config.decision.themeCheck), publicVerification: 'pending' });
  } catch (error) {
