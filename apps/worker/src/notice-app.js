...
call('GET').then(d=>{form.elements.enabled.checked=d.input?.enabled===true;form.elements.goods.checked=d.input?.sellsGoodsToConsumers===true;message.textContent=describe(d.status);save.disabled=false;}).catch(()=>{message.textContent='No se ha podido conectar con Shopify. Abre esta app desde la administración con la cuenta propietaria.';});`;
