...
## Read-only baseline inspection (2026-09-16)

The DEV owner can explicitly request GET /app/theme-observation from the technical details panel. It uses the same server-side theme query as invalidation, returning only validated IDs, timestamps, pagination completeness, counts and the SHA-256 of the exact Header file bytes. It never returns theme contents or credentials, writes metafields, accepts a review or changes publication. Observations remain reviewed:false and publicVerification:pending; a real placement review is required before an operator configures a baseline. The endpoint inherits the existing DEV-only app settings and owner session-token exchange. The comparison guard remains disabled.
